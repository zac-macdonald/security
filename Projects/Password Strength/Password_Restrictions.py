...
Here's an example senario:
    ...
    8 characters in length, and contains three numbers. She can't use the password "password" anymore,
    so she uses "password111" instead. The problem with this is that inherently, the password is still quite easy to guess.
    Humans by nature take the path of least resistance, hence the subtle change in the password.

    So, an attacker now wants to hack the bank again, and this time, the attacker sees the password policy.
    Now, instead of the attacker trying to brute force a password with all combinations of characters, 
    less than the length of 8, the attacker can try to brute force the password with all combinations of characters,
    more than the length of 8, and also contains three numbers.
    To put it simply, the attacker has just narrowed the amount of passwords that they have to check, 
    making it easier for the attacker to crack the password.
    ...
    Which means, that if the password length is 8, the number of possible characters is 94,
    and total combination of passwords for the given password length is 6095689385410816,
    the time it would take to crack the password, is around 1 hour and 45 minutes. (Calculated below)
"""
